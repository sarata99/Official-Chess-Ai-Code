...
var makeMove = function(algo, skill=3) {
  ...
  if (algo === 1) {
    ...
    var move = calcBestMove1(skill, game, game.turn())[1];
  } else if(algo === 5){
    var move = calcBestMove2(skill, game, game.turn())[1];
  } else if(algo === 6){
    var move = calcBestMove3(skill, game, game.turn())[1];
  }
...
var playGame = function(algoB =4, algoW=5, skillW=1, skillB=1) {
  if (game.game_over() === true) {
    console.log('game over');
	game.reset()
    return;
  ...
  makeMove(algo, skill);
  window.setTimeout(function() {
    playGame(algoB, algoW, skillW, skillB);
  }, 250);
};
